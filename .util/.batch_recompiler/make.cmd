...
FOR /F %%a IN ('DIR /b "%todoDir%\*.c"') DO (
	...
	MsBuild /nologo /verbosity:normal /p:Configuration=Release;Platform=Win32 "%tempDir%\%%~na\%%~na.vcxproj" > "%logDir%\%%~na.x86.log"
	MsBuild /nologo /verbosity:normal /p:Configuration=Release;Platform=x64 "%tempDir%\%%~na\%%~na.vcxproj" > "%logDir%\%%~na.x64.log"
