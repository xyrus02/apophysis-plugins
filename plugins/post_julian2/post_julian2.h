...
{
	VAR(absN) = (int)abs(VAR(post_julian2_power));
    VAR(cN) = VAR(post_julian2_dist) / VAR(post_julian2_power) / 2;
...
int PluginVarCalc(Variation* vp)
{
    ...
    double angle = (atan2(y, x) + M_2PI * (rand() % (int)VAR(absN)))/ VAR(post_julian2_power);
